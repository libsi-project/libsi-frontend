...
/// A context associated to an [InheritedProvider].
///
/// It provides access to the current value and an extra [markNeedsNotifyDependents] method.
///
/// Note: This does not extend [BuildContext] as BuildContext is sealed in newer versions.
/// Implementations of this interface will have BuildContext functionality through their
/// inheritance chain (e.g., _InheritedProviderScopeElement extends InheritedElement).
abstract class InheritedContext<T> {
...
''');
    ...
    // Call notifyClients if needed after update (the updated() method was removed in newer Jaspr)
    if (_updatedShouldNotify) {
      notifyClients(component as InheritedComponent);
    }
